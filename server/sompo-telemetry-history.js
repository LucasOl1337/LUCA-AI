...
import { SOMPO_AGRI_EQUIPMENT, getSompoAgriScenario } from '../shared/sompo-agri-scenarios.js';
import { getSompoAgriPosition, isSompoAgriScenarioId } from '../shared/sompo-agri-brief.js';
import { getSompoGeofenceSite, SOMPO_GEOFENCE_SITE_VERSION } from '../shared/sompo-geofence-sites.js';
import { computeGeofenceEpisodes } from '../shared/lab-geofence.js';
import { evaluateGeofence } from '../shared/sompo-geofence.js';
...
export function summarizeSompoEpisodeSamples(samples, episode = null) {
  return { ...summarizeEpisodeMotion(samples), geofence: summarizeEpisodeGeofence(samples, episode) };
}
 ...
 */
function summarizeEpisodeGeofence(samples, episode) {
  ...
  if (!site) return null;
  const machine = SOMPO_AGRI_EQUIPMENT[scenario.equipmentId];
  const originMs = samples[0].observedMs;
