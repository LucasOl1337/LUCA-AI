...
test('snapshot agrícola: proveniência correta, roll amplo e gravidade rotacionada', () => {
  ...
  assert.deepEqual(rolled.risks, { collision: true, inclination: true }, 'cenário sem talhão não ganha campo do geofencing (módulo isolado)');
  assert.equal('geofence' in rolled, false);
...
test('plano de episódio agrícola: todo desfecho é roteirizado e capturável', () => {
  for (const scenario of Object.values(SOMPO_AGRI_SCENARIOS)) {
    for (const outcome of getSompoAgriOutcomes(scenario.scenarioId)) {
      ...
      const scriptedOutcome = scenario.outcomes[outcome.id];
      assert.equal(plan.phases, scriptedOutcome.phases ?? scenario.phases);
