...
const TEXT_EXTENSIONS = new Set([
  ...
  '.js', '.jsx', '.ts', '.tsx', '.css', '.html', '.htm', '.py', '.rb', '.go', '.rs', '.java', '.c', '.h', '.cpp', '.hpp',
]);
const TEXT_MIME_TYPES = new Set([
  'text/plain', 'text/markdown', 'text/csv', 'text/xml', 'text/yaml',
  'application/json', 'application/xml', 'application/yaml', 'application/x-yaml', 'application/toml', 'application/sql',
...
function attachmentDir(userId, sessionId) {
  return path.join(workspacesRoot, safeUserDir(userId), 'chat-attachments', safeSessionDir(sessionId));
}
...
function cleanName(value) {
  const name = path.basename(String(value || '').replaceAll('\\', '/')).replace(/[\u0000-\u001f\u007f]/g, '').trim();
...
function hasImageSignature(buffer, mimeType) {
  ...
  if (mimeType === 'image/jpeg') return buffer.length >= 3 && buffer.subarray(0, 3).equals(Buffer.from('ffd8ff', 'hex'));
...
function normalizeAttachmentType(name, requestedMimeType, buffer) {
  ...
  if (IMAGE_MIME_TYPES.has(requested) || detectedImage) {
    const mimeType = requested || detectedImage;
  ...
  if (TEXT_MIME_TYPES.has(requested) || TEXT_EXTENSIONS.has(extension)) {
    if (!validateText(buffer)) throw attachmentError('attachment_text_invalid');
    return { kind: 'text', mimeType: requested && TEXT_MIME_TYPES.has(requested) ? requested : 'text/plain' };
  }
...
/**
 * Drop the attachment directories of sessions that no longer exist — the session
 * row is already gone, so getChatSession() can no longer resolve them.
 * Used by cascade folder deletion, which removes many sessions at once.
 */
...
/** Test/ops helper: which sessions still own an attachment directory on disk. */
...
// Cascade folder deletion drops many sessions at once; without this the private
// files of those sessions would stay on disk with no owner able to reach them.
...
export function resolveChatAttachmentsForModel(sessionId, attachmentIds = []) {
  const userId = requireWorkspaceUserId();
  const session = getChatSession(String(sessionId || '').trim());
  const ids = [...new Set((Array.isArray(attachmentIds) ? attachmentIds : []).map(String).map((id) => id.trim()).filter(Boolean))];
  ...
  return ids.map((id) => {
    ...
    if (totalBytes > MAX_CHAT_ATTACHMENTS_TOTAL_BYTES) throw attachmentError('attachments_total_too_large', 413);
