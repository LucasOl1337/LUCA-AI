...
import { sompoEpisodeSampleOffsets, createSompoSimulationSnapshot } from '../shared/sompo-telemetry-simulator.js';
...
test('delayed script ticks retain every scheduled sample including the final point', () => {
  const offsets = [];
  for (const elapsed of [50, 790, 2300, 16000, 25000]) offsets.push(...sompoEpisodeSampleOffsets(offsets.at(-1) ?? -Infinity, elapsed, 500, 22_000));
