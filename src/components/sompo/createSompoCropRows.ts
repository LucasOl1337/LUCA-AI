...
export function createSompoCropRows(groundHeight: (x: number, z: number) => number, compact: boolean, height = 1) {
  ...
  material.onBeforeCompile = shader => {
    ...
    shader.vertexShader = `uniform float cropTime,cropWind; uniform vec3 cropCut; uniform vec2 cropPath[8]; varying float cropHeight; varying float cropTint; varying float cropStubble;\n` + shader.vertexShader
  ...
  material.customProgramCacheKey = () => `sompo-threeui-leafpair-v1-${height}`;
  ...
  return {
    ...
    dispose() { leafTexture.value?.dispose(); material.dispose(); },
  };
}
