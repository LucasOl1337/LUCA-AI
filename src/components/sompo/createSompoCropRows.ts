...
export function createSompoCropRows(groundHeight: (x: number, z: number) => number, compact: boolean, height = 1) {
  ...
  material.onBeforeCompile = shader => {
    Object.assign(shader.uniforms, { cropTime: time, cropWind: wind, cropCut: cut, cropPath: path });
    shader.vertexShader = `uniform float cropTime,cropWind; uniform vec3 cropCut; uniform vec2 cropPath[8]; varying float cropHeight; varying float cropTint;\n` + shader.vertexShader
      .replace('#include <beginnormal_vertex>', `
        ...
        float bend=(.06+cropWind*.16*(.55+.45*w2)*(.55+.45*w1))*pow(max(h,.001),1.55)*${height.toFixed(2)};
      ...
      .replace('#include <begin_vertex>', `
        float width=${height < .5 ? '.08' : '.16'}*max(.02,1.-h*.92);
    ...
    shader.fragmentShader = 'varying float cropHeight; varying float cropTint;\n' + shader.fragmentShader.replace('#include <color_fragment>', `#include <color_fragment>
      ...
      diffuseColor.rgb*=mix(base,tip,pow(max(cropHeight,.001),1.2))*(.78+cropTint*.44);`);
  };
  material.customProgramCacheKey = () => `sompo-threeui-ribbon-v4-${height}`;
  ...
  for (let tile = 0; tile < 8; tile++) {
    ...
    for (let row = 0; row < rows; row++) for (let col = 0; col < columns; col++) {
      ...
      dummy.rotation.set(0, 0, 0); dummy.scale.set(1, height * (.78 + (Math.sin(seed) * .5 + .5) * .45), 1); dummy.updateMatrix();
  ...
  return {
    root,
    setHarvestPath(points: readonly { x: number; z: number }[]) {
