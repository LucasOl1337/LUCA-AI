...
/** Deterministic per-plant tint: no two neighbours share the same crown or tuft. */
function plantTint(i: number, tree: boolean) {
  const hue = random(i + 301) - .5, value = random(i + 607) - .5, dry = random(i + 911);
  const color = new THREE.Color(1, 1, 1);
  // Crowns range from deep green to a sun-bleached olive; a few carry the dry-season yellow.
  if (tree) {
    const green = 1 + value * .3 + (dry > .82 ? .1 : 0);
    // Blue stays under green: a cool crown reads deep green, never teal.
    color.setRGB(1 + hue * .42 + value * .34 + (dry > .82 ? .22 : 0), green, green * (.94 - Math.abs(hue) * .3 - (dry > .82 ? .25 : 0)));
  }
  else if (dry > .8) color.setRGB(1.3 + value * .2, 1.14 + value * .16, .72);
  else color.setRGB(1 + hue * .12 + value * .3, 1 + value * .28, 1 - hue * .1 + value * .2);
  return color;
}
...
export function createSompoLicensedFoliage(parent: THREE.Group, camera: THREE.Camera) {
  ...
  const batches: { meshes: THREE.InstancedMesh[]; slots: FoliageSlot[]; min: number; max: number; thinFrom?: number; base: THREE.Matrix4[]; tints: THREE.Color[]; varied: boolean[] }[] = [];
  const geometries = new Set<THREE.BufferGeometry>(), materials = new Set<THREE.Material>(), textures = new Set<THREE.Texture>();
  const trees = highwayFoliageSlots('trees');
  // Phones render the same framing at a third of the pixels: half the ground
  ...
  const pending = specs.map(async spec => {
    ...
    try {
      ...
      const meshes: THREE.InstancedMesh[] = [], base: THREE.Matrix4[] = [], varied: boolean[] = [];
      model.traverse(node => {
        ...
        for (const material of Array.isArray(mesh.material) ? mesh.material : [mesh.material]) {
          ...
          if (material instanceof THREE.MeshStandardMaterial) {
            if ('tint' in spec && spec.tint) material.color.setRGB(...spec.tint);
            ...
            if (material.transparent || /leav|grass|shrub/i.test(material.name)) {
              ...
              // Coverage from the MSAA target: leaf edges resolve to 4 sub-samples
              // instead of a per-pixel cut that shimmers as speckle at distance.
              material.alphaToCoverage = true;
              // Coverage keeps partial texels that the cut used to drop; deepen the
              // albedo so crowns keep their mass instead of reading pale.
              if (!('tint' in spec && spec.tint)) material.color.multiply(new THREE.Color(.84, .9, .74));
        ...
        // Trunk and branches keep their bark; leaves and blades take the per-plant tint.
        varied.push(!Array.isArray(mesh.material) && /leav|grass|shrub|Plane/i.test(`${mesh.material.name} ${mesh.name}`));
      });
      batches.push({ meshes, base, varied, slots: spec.slots, min: spec.min, max: spec.max, thinFrom: spec.thinFrom, tints: spec.slots.map((_, i) => plantTint(i, spec.slots === trees)) });
    ...
    update(_wet: boolean, truckX = 0, focus = new THREE.Vector3(truckX, 1.5, 0)) {
      ...
      for (const batch of batches) {
        ...
        for (let index = 0; index < batch.slots.length; index++) {
          ...
          if (compact && batch.slots !== trees) {
            if (random(index + 733) > .5) continue;
            scale *= 1.3;
          }
