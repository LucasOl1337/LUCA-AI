...
export default function SompoFleetPanel({ telemetry, episodeVersion, onRefreshTelemetry }: Props) {
  ...
  async function launch() {
    ...
    try {
      ...
      queueSompoLaunch({ caseId: 'safra-desempenho-frota', mission, mode: 'team', presetId: 'conselho-estrategia', presetLabel: 'Conselho de Estratégia', autoRun: true, attachments: [upload.attachment] });
  ...
  return <section className="sompo-fleet" aria-labelledby="sompo-fleet-title">
    <header className="sompo-fleet-heading"><div><span className="sompo-fleet-kicker">Safra · Desempenho da frota</span><h2 id="sompo-fleet-title">O que mudar na próxima safra?</h2><p>Compare o que cada máquina viveu e leve as evidências para o Conselho de Estratégia decidir o próximo passo.</p></div><button disabled={busy} onClick={() => { void load(); void onRefreshTelemetry(); }}><RefreshCw size={16} />{busy ? 'Atualizando…' : 'Atualizar'}</button></header>
