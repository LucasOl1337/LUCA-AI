...
import { parseLabCase, parseLabSite, associateLabSite, getReplayFrame, formatLabTime, type LabCase, type LabSite } from '../../shared/lab-telemetry.js';
...
import { LAB_EXAMPLES, exampleMetadata, loadDefaultLabSite, categories, labRequest, downloadFile, exportLabReport, type CaseSummary, type SavedCase, type LabAnalysis, type ConclusionDraft, type Category, type Hypothesis } from '@/lib/lab-client';
...
export default function LaboratorioPage() {
  ...
  async function openExample(example: typeof LAB_EXAMPLES[number]) {
    ...
    try {
      ...
      const [manifestResponse, mapResponse, schemaResponse] = await Promise.all([fetch(exampleMetadata.manifest), fetch(exampleMetadata.map), fetch(exampleMetadata.schema)]);
      if (![manifestResponse, mapResponse, schemaResponse].every(item => item.ok)) throw new Error('Os metadados do exemplo não estão disponíveis. Tente novamente.');
      await openCsv(await response.text(), example.fileName, { manifest: await manifestResponse.json(), map: await mapResponse.json(), schema: await schemaResponse.json() });
    }
  ...
  const currentWarnings = frame?.gap ? 'GPS indisponível · posição não reconstruída' : frame?.activeEvents.some(event => event.type === 'coolant_warning') ? 'Temperatura acima do limite didático' : frame?.activeEvents.some(event => event.type === 'near_water') ? 'Máquina próxima da zona de água' : frame?.activeEvents.some(event => event.type === 'outside_fence') ? 'Máquina fora da área permitida' : 'Operação dentro das regras verificáveis';
  ...
  return <div className="lab-page lab-scene-first" data-lab-page data-panel-open={panelOpen} data-scene-details={sceneDetails}>
    ...
    <div className="lab-workbench">
      <section className={`lab-viewport ${sensorView ? 'lab-viewport-sensors' : ''}`} aria-label={sensorView ? 'Replay dos sensores ESP32' : 'Mapa e replay do caso'}>
        ...
        {!labCase && !sceneDetails && <div className="lab-demo-start"><strong>Veja a máquina no campo</strong><p>Acompanhe um percurso de demonstração na fazenda.</p><button disabled={!farmSite || !!busy} onClick={openFarmDemo}><Play size={16} />Ver caminhão em movimento</button><small>Percurso sintético · cenário real</small></div>}
      ...
      <aside className="lab-panel" id="lab-case-panel" hidden={!panelOpen} aria-label={STEPS[step]}>
        ...
        <div className="lab-panel-body">
          ...
          {(step === 0 || step === 1) && <details className="lab-details lab-area">
            ...
            <label className="lab-field">Área para associar<select aria-label="Área para associar" value={siteChoice} disabled={!!busy || saving} onChange={event => setSiteChoice(event.target.value)}><option value="farm">Frying Pan Farm Park · EUA</option>{customSite && <option value="custom">{customSite.manifest?.site?.name || 'Área carregada'}</option>}<option value="none">Somente arquivos do caso</option></select></label>
          ...
          {step === 0 && <>
            <button className="lab-demo-library" disabled={!farmSite || !!busy} onClick={openFarmDemo}><Play size={16} />Caminhão na fazenda <small>Demonstração sintética · 90 s</small></button>
            ...
            <div className="lab-examples">{LAB_EXAMPLES.map((example, index) => <button key={example.kind} disabled={!!busy} onClick={() => void openExample(example)} data-lab-example={example.kind}><span className={`lab-example-icon ${example.kind}`}>{busy === example.kind ? <LoaderCircle className="lab-spin" size={19} /> : index === 0 ? <Tractor size={20} /> : index === 1 ? <Waves size={20} /> : <Thermometer size={20} />}</span><span><strong>{example.title}</strong><small>{example.description}</small></span><ArrowRight size={16} /></button>)}</div>
